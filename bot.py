...
from handlers.analyze_handler import analyze, show_details_callback ##########
...
def main():
    ...
    app.add_handler(ConversationHandler(entry_points=[CommandHandler("analyze", analyze)],
        states={
            STAGE_SYMBOL: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_symbol)],
            STAGE_TIMEFRAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_timeframe)],
            STAGE_CAPITAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_capital)],
            STAGE_RISK: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_risk)],
            STAGE_LEVERAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_leverage)],
        },
        fallbacks=[],
    )
)
